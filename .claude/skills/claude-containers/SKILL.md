...
| `bin/claude-launch` | Create/start a container; auto port; labels carry metadata; surfaces a fast-failing entrypoint. `--browser` enables the chrome-devtools MCP for frontend debugging (requires WITH_BROWSER=1 image). |
...
./bin/claude-launch <name> [--port N] [--mcp foo --mcp bar] [--browser] [--extra-args "…"]
